...
import numpy as np
import torch
from common.misc_utils import POSE_CSV_HEADER, EpisodeRunner
...
def visualize_rl_controller_replay(args):
    ...
    actor_critic = torch.load(controller_path, map_location=device, weights_only=False)
    ...
    with EpisodeRunner(env, save=args.save, max_steps=args.len, csv=args.csv) as runner:
        ...
        while not runner.done:
            ...
            if done.any():
                ...
                ep_reward *= torch.logical_not(done).float()
