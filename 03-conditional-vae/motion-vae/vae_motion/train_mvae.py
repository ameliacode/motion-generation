...
class StatsLogger:
    ...
    def log_stats(self, data):
        ...
        self.writer.add_scalar("Loss/Total", ep_recon_loss + ep_kl_loss, ep)
...
def main():
    ...
    args = SimpleNamespace(
        ...
        mocap_file=os.path.join(data_path, "03_data.npz"),
        norm_mode="zscore",
        latent_size=32,
        num_embeddings=12,
        num_experts=6,
        num_condition_frames=1,  # COMMENT |
        num_future_predictions=1,
        num_steps_per_rollout=8,
        kl_beta=0.4,  # COMMENT | 0.2 in paper
    ...
    try:
        for ep in tqdm(range(1, args.num_epochs + 1), desc="train mvae"):
            ...
            num_mini_batch = 1
            for num_mini_batch, indices in tqdm(enumerate(sampler), desc="mini batch"):
                ...
                history[:, : args.num_condition_frames].copy_(
                    mocap_data[condition_range]
                )
                ...
                for offset in range(args.num_steps_per_rollout):
                    ...
                    prediction_range = (
                        ...
                        + torch.arange(
                            offset, offset + args.num_future_predictions
                        ).long()
                    )
