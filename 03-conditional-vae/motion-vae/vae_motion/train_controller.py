...
from common.misc_utils import update_exponential_schedule, update_linear_schedule
...
class StatsLogger:
    def __init__(self, csv_path, tensorboard_dir=None):
    ...
    def log_stats(self, args, data):
        ...
        # Log to TensorBoard
        update = data["update"]
        self.writer.add_scalar("Reward/Mean", data["mean_rew"], update)
        self.writer.add_scalar("Reward/Median", data["median_rew"], update)
        self.writer.add_scalar("Reward/Min", data["min_rew"], update)
        self.writer.add_scalar("Reward/Max", data["max_rew"], update)
        self.writer.add_scalar("Loss/Value", data["value_loss"], update)
        self.writer.add_scalar("Loss/Policy", data["action_loss"], update)
        self.writer.add_scalar("Loss/Entropy", data["dist_entropy"], update)
        self.writer.add_scalar(
            "Training/FPS", int(data["num_steps"] / (now - self.start)), update
        )
        self.writer.add_scalar("Training/Timesteps", data["num_steps"], update)
    ...
    try:
        for update in tqdm(range(args.num_updates)):
            ...
            for step in range(args.num_steps_per_rollout):
                ...
                masks = torch.logical_not(done).float()
                bad_masks = (~(done * end_of_rollout)).float()

                if done.any():
                    ep_info["reward"].append(ep_reward[done].clone())
                    ep_reward *= torch.logical_not(done).float()  # zero out the dones
