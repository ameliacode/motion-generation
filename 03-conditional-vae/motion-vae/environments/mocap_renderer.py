...
from common.bullet_objects import Arrow, FlagPole, VCapsule, VCylinder, VSphere
...
class PBLMocapViewer:
    def __init__(
        ...
        indices = torch.arange(0, 96).long()  # refactor # CMU | 96 # MVAE | 69
        self.x_indices = (
            indices[slice(3, 96, 3)] if x_ind is None else x_ind
        )  # Mocap data refactor
...
class MultiMocapCharacters:
    ...
    def set_joint_positions(self, xyzs, index):
        ...
        if self.has_links:
            ...
            self.heads[index].set_position(xyzs[16])  # CMU 16 | LAFAN 13
