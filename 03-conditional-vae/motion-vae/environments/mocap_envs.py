...
class EnvBase(gym.Env):
    def __init__(
        ...
        # Fix: Use integers() instead of randint() for newer NumPy versions
        if hasattr(self.unwrapped.np_random, "integers"):
            indices = self.unwrapped.np_random.integers(
                0, self.mocap_data.shape[0], self.num_parallel
            )
        else:
            indices = self.unwrapped.np_random.randint(
                0, self.mocap_data.shape[0], self.num_parallel
            )
        ...
        self.foot_xy_ind = torch.LongTensor(
            [[30, 32], [15, 17]]
        )  # joint index refactored #CMU
        ...
        indices = torch.arange(0, 96).long().to(self.device)  # 96 | CMU # 69 | LFAN
        x_indices = indices[slice(3, 96, 3)]  # 96
        y_indices = indices[slice(4, 96, 3)]  # 96
        z_indices = indices[slice(5, 96, 3)]  # 96
    ...
    def load_data(self, pose_vae_path):
        ...
        self.pose_vae_model = torch.load(
            pose_vae_path, map_location=self.device, weights_only=False
        )
    ...
    def calc_energy_penalty(self, next_frame):
        action_energy = (
            ...
            + next_frame[:, 96:189]
            .pow(2)
            .mean(1)  # this needs to be refactored # LAFAN | 69:135 # CMU | 96:189
        )
...
class RandomWalkEnv(EnvBase):
    ...
    def dump_additional_render_data(self):
        ...
        )  # this has to be refactored # 0:69 135:267 # 0:96 189:375
    ...
    def reset_target(self, indices=None, location=None):
        ...
        if self.timestep % facing_switch_every == 0:
            ...
            if self.is_rendered:
                ...
                self.target_direction = self.unwrapped.np_random.uniform(0, 2 * np.pi)
            else:
        ...
        if self.timestep % speed_switch_every == 0:
            if self.is_rendered:
                # in rendered mode, we use these because they are linked to gui
                self.target_speed = self.unwrapped.np_random.choice(
                    np.linspace(0, 0.8, 9)
                )
            else:
                # in training mode, we have tensors
